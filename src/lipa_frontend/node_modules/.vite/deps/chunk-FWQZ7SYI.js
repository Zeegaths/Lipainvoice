...
function weierstrassN(CURVE, curveOpts = {}) {
  const { Fp: Fp4, Fn: Fn2 } = _createCurveFields("weierstrass", CURVE, curveOpts);
  ...
  function pointFromBytes(bytes) {
    ...
    if (length === LC && (head === 2 || head === 3)) {
      ...
      const x2 = Fp4.fromBytes(tail.subarray(L2 * 0, L2 * 1));
      const y2 = Fp4.fromBytes(tail.subarray(L2 * 1, L2 * 2));
      if (!isValidXY(x2, y2))
  ...
  const assertValidMemo = memoized((p2) => {
    if (p2.is0()) {
      if (curveOpts.allowInfinityPoint && !Fp4.is0(p2.Y))
        return;
...
function tower12(opts) {
  ...
  const Fp22 = {
    ...
    sqrt: (num) => {
      ...
      let d2 = Fp4.mul(Fp4.add(a2, c0), Fpdiv2);
    ...
    cmov: ({ c0, c1 }, { c0: r0, c1: r1 }, c2) => ({
      c0: Fp4.cmov(c0, r0, c2),
      c1: Fp4.cmov(c1, r1, c2)
    }),
...
var bls12_381_Fr = Field(bls12_381_CURVE_G1.n, { modOnDecode: true });
...
var _Certificate = class _Certificate {
  ...
  static createUnverified(options) {
    return new _Certificate(options.certificate, options.rootKey, options.canisterId, options.blsVerify ?? blsVerify, options.maxAgeInMinutes, options.disableTimeVerification);
  }
  /**
  ...
  async _checkDelegationAndGetKey(d2) {
    ...
    if (this._canisterId.toString() !== MANAGEMENT_CANISTER_ID) {
      ...
      if (!canisterInRange) {
        throw TrustError.fromCode(new CertificateNotAuthorizedErrorCode(this._canisterId, d2.subnet_id));
      }
    }
    const publicKeyLookup = lookupResultToBuffer(cert.lookup_path(["subnet", d2.subnet_id, "public_key"]));
    if (!publicKeyLookup) {
      throw TrustError.fromCode(new MissingLookupValueErrorCode(`Could not find subnet key for subnet 0x${bytesToHex(d2.subnet_id)}`));
    }
...
function edwards(CURVE, curveOpts = {}) {
  const { Fp: Fp4, Fn: Fn2 } = _createCurveFields("edwards", CURVE, curveOpts);
  const { h: cofactor, n: CURVE_ORDER } = CURVE;
  _validateObject(curveOpts, {}, { uvRatio: "function" });
  const MASK = _2n6 << BigInt(Fn2.BYTES * 8) - _1n8;
  const modP = (n) => Fp4.create(n);
  const uvRatio2 = curveOpts.uvRatio || ((u, v2) => {
    try {
  ...
  class Point {
    ...
    multiply(scalar) {
      ...
      const { p: p2, f } = wnaf.cached(this, n, (p3) => normalizeZ(Point, p3));
    ...
    multiplyUnsafe(scalar, acc = Point.ZERO) {
      ...
      if (this.is0() || n === _1n8)
        return this;
      return wnaf.unsafe(this, n, (p2) => normalizeZ(Point, p2), acc);
    }
    ...
    static fromHex(hex, zip215 = false) {
      ...
      aInRange("pointHex.y", y2, _0n8, max);
      const y22 = modP(y2 * y2);
    ...
    toBytes() {
      ...
      const bytes = numberToBytesLE(y2, Fp4.BYTES);
...
function eddsa(Point, cHash, eddsaOpts) {
  ...
  const { BASE: G2, Fp: Fp4, Fn: Fn2 } = Point;
...
function _eddsa_new_output_to_legacy(c2, eddsa2) {
  const legacy = Object.assign({}, eddsa2, { ExtendedPoint: eddsa2.Point, CURVE: c2 });
  return legacy;
}
...
var _RistrettoPoint = class __RistrettoPoint extends PrimeEdwardsPoint {
  ...
  static fromBytes(bytes) {
    abytes(bytes, 32);
    const { a: a2, d: d2 } = ed25519.CURVE;
    const P2 = Fp3.ORDER;
    const mod2 = Fp3.create;
    const s2 = bytes255ToNumberLE(bytes);
    if (!equalBytes(numberToBytesLE(s2, 32), bytes) || isNegativeLE(s2, P2))
...
var _HttpAgent = class _HttpAgent {
  ...
  constructor(options = {}) {
    ...
    __privateSet(this, _timeDiffMsecs, 0);
  ...
  async query(canisterId, fields, identity) {
    ...
    const makeQuery = async () => {
      return {
        ...
        query: await __privateMethod(this, _requestAndRetryQuery, requestAndRetryQuery_fn).call(this, args)
      };
    };
    const getSubnetStatus = async () => {
      ...
      return __privateGet(this, _subnetKeys).get(ecid.toString());
    };
    try {
      ...
      try {
        ...
        __privateGet(this, _subnetKeys).delete(canisterId.toString());
        await this.fetchSubnetKeys(ecid.toString());
        const updatedSubnetStatus = __privateGet(this, _subnetKeys).get(canisterId.toString());
    ...
    __privateSet(this, _syncTimePromise, __privateGet(this, _syncTimePromise) ?? (async () => {
      ...
      try {
        ...
        if (maxReplicaTime > BigInt(0)) {
          __privateSet(this, _timeDiffMsecs, Number(maxReplicaTime) - Number(callTime));
          this.log.notify({
...
syncTimeGuard_fn = async function(canisterIdOverride) {
  if (__privateGet(this, _shouldSyncTime) && __privateGet(this, _timeDiffMsecs) === 0) {
...
@noble/curves/esm/ed25519.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-FWQZ7SYI.js.map
