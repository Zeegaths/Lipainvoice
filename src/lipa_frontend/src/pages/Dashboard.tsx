...
const Dashboard = ({ onNavigate }: DashboardProps) => {
  ...
  const transformInvoices = (
    backendInvoices: Array<[bigint, BackendInvoice]>
  ): Invoice[] => {
    ...
    return (
      <div className="flex items-center justify-center h-screen w-screen">
        <div className="text-center">
          <p className="text-red-500 text-lg mb-4">Error loading invoices</p>
  ...
  const filterByStatus = (status: string) => {
    ...
    if (status === "all") {
      const transformed = backendInvoices
        ? transformInvoices(backendInvoices)
        : [];
      setFilteredInvoices(transformed);
      return;
    }
    ...
    const filtered = transformed.filter(
      (invoice: Invoice) => invoice.status.toLowerCase() === status
    );
    setFilteredInvoices(filtered || []);
  };

  const transformedInvoices = backendInvoices
    ? transformInvoices(backendInvoices)
    : [];
  ...
  const totalUnpaid = transformedInvoices
    .filter(
      (inv: Invoice) => inv.status === "unpaid" || inv.status === "pending"
    )
    .reduce((sum: number, inv: Invoice) => sum + inv.amount, 0);
  const totalOverdue = transformedInvoices
        ...
        <div className="mb-8">
          ...
          <div className="mb-4 mt-2">
            ...
            <p className="text-sm text-gray-400">
              Amount earned in BTC since May 2025
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6">
            <div className="bg-white rounded-3xl border border-gray-300 flex items-center gap-4 px-4 py-2">
              ...
              <div>
                ...
                <p className="text-4xl font-mono lg:text-2xl font-bold text-gray-900">
            ...
            <button
              ...
              className={`${
                activeTab === "all"
                  ...
                  : null
              } text-sm font-medium pb-2 whitespace-nowrap`}
            >
              All Invoices
            </button>
            <button
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredInvoices &&
                    filteredInvoices.slice(0, 7).map((invoice, index) => (
                      <tr key={invoice.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          @{invoice.client.toLowerCase().replace(/\s+/g, "")}
                          @gmail.com
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {invoice.date}
