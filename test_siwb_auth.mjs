...
const didPath = path.resolve('packages/ic_siwb_provider/ic_siwb_provider.did');
const didContent = fs.readFileSync(didPath, 'utf8');

// Simple IDL factory for the SIWB provider service
const idlFactory = ({ IDL }) => {
  return IDL.Service({
    'generate_challenge': IDL.Func([], [IDL.Text], []),
    'verify_signature': IDL.Func([IDL.Text, IDL.Text, IDL.Text], [IDL.Bool], []),
    'ping': IDL.Func([], [IDL.Text], []),
    'get_service_info': IDL.Func([], [IDL.Record({
      'name': IDL.Text,
      'version': IDL.Text,
      'status': IDL.Text
    })], ['query']),
    'set_status': IDL.Func([IDL.Text], [IDL.Record({
      'name': IDL.Text,
      'version': IDL.Text,
      'status': IDL.Text
    })], [])
...
async function testFrontendIntegration() {
    ...
    try {
        // Test that the frontend utilities file exists and is readable
        const utilsPath = path.resolve('src/lipa_frontend/src/utils/signatureVerification.ts');
        const configPath = path.resolve('src/lipa_frontend/src/config/canisterConfig.ts');

        if (fs.existsSync(utilsPath)) {
            console.log('✅ Frontend utilities file exists');
        } else {
            console.log('❌ Frontend utilities file not found');
            return false;
        }

        if (fs.existsSync(configPath)) {
            console.log('✅ Canister configuration file exists');
        } else {
            console.log('❌ Canister configuration file not found');
            return false;
        }

        // Test that canister config is accessible
        const configContent = fs.readFileSync(configPath, 'utf8');
        if (configContent.includes('ic_siwb_provider')) {
            console.log('✅ Canister configuration contains SIWB provider');
        } else {
            console.log('❌ Canister configuration missing SIWB provider');
            return false;
        }
